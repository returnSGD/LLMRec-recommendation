...
def train_epoch(model: BaseP5Model, dataloader: DataLoader,
                ...
                use_sample_engineering: bool = True,
                use_amp: bool = False,
    ...
    for batch in pbar:
        ...
        # Forward pass with mixed precision (bf16 preferred — stable on Blackwell)
        with torch.amp.autocast('cuda', dtype=amp_dtype, enabled=use_amp):
            outputs = model(input_ids, attention_mask, labels=labels,
                            output_hidden_states=use_sample_engineering)
        ce_loss = outputs['loss']

        # ── NaN guard: detect first occurrence and pinpoint source ──
        def _check_nan(tensor, name):
            if torch.isnan(tensor).any() or torch.isinf(tensor).any():
                nan_count = torch.isnan(tensor).sum().item()
                inf_count = torch.isinf(tensor).sum().item()
                print(f"\n[NaN DETECTED] {name} at batch {num_batches+1}")
                print(f"  NaN count: {nan_count}, Inf count: {inf_count}, "
                      f"tensor stats: min={tensor.min().item():.4f} max={tensor.max().item():.4f}")
                if hasattr(tensor, 'dtype'):
                    print(f"  dtype: {tensor.dtype}")
                if 'logits' in outputs:
                    l = outputs['logits']
                    print(f"  logits shape: {l.shape}, "
                          f"range: [{l.min().item():.4f}, {l.max().item():.4f}], "
                          f"NaN: {torch.isnan(l).any().item()}, Inf: {torch.isinf(l).any().item()}")
                print(f"  Sample prompts (first 100 chars): {[p[:100] for p in prompts[:3]]}")
                return True
            return False

        nan_detected = False
        nan_detected |= _check_nan(ce_loss, "CE loss")
        if 'seq_embedding' in outputs:
            nan_detected |= _check_nan(outputs['seq_embedding'], "seq_embedding")
            ...
            if sans_sampler and 'seq_embedding' in outputs:
                ...
                if pos_embs_list:
                    ...
                    if all_neg_embs:
                        ...
                        nan_detected |= _check_nan(infonce_loss, "InfoNCE loss")
            ...
            if recaug_pipeline and 'seq_embedding' in outputs:
                ...
                if aug_variants:
                    ...
                    if 'seq_embedding' in aug_outputs:
                        ...
                        nan_detected |= _check_nan(consistency_loss, "RecAug consistency loss")
...
def main():
    ...
    # Mixed precision: bf16 (safe, Blackwell-native) or fp16 (legacy, may overflow on CUDA 13+)
    # bf16 has same exponent range as fp32 — no overflow, no GradScaler needed
    use_amp = config['training'].get('fp16', False) or config['training'].get('bf16', False)
